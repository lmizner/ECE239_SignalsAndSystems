...
def check_bigram(model, checkpoint_file, device="cpu"):
    ...
    absolute_diff = torch.abs(check_output - old_output)
    
    # Check the maximum absolute difference
    max_absolute_diff = torch.max(absolute_diff)
    
    # Print or inspect the maximum absolute difference
    print("Maximum absolute difference:", max_absolute_diff.item())
