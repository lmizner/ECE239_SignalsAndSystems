...
class DQN:
    ...
    def _optimize_model(self):
        ...
        # Compute target Q-values directly using the same model
        with torch.no_grad():
            target_q_values_next = self.model(next_states).max(1, keepdim=True)[0]
        ...
        # Print shapes
        # print("Shapes - q_values:", q_values.shape, "target_q_values:", target_q_values.shape)
...
class HardUpdateDQN(DQN):
    ...
    def __init__(self, env, model, model_kwargs: dict = {},
             update_freq: int = 100, seed: int = 42,
        ...
        super().__init__(env, model, model_kwargs, update_freq, *args, **kwargs)
        
        # Extract input_size and action_size from environment
        input_size = env.observation_space.shape  # Shape of observation space
        action_size = env.action_space.n  # Number of actions
        
        # Initialize the target model with the same architecture as the main model
        self.target_model = model(input_size = input_size, action_size = action_size, **model_kwargs).to(device)
    ...
    def _optimize_model(self):
        ...
        # # Compute Q-values for current states and actions using the online model
        # q_values = self.model(states).gather(1, actions.unsqueeze(1))
    
        # # Select actions using the online model for the next states
        # next_actions = self.model(next_states).argmax(1, keepdim=True)
    
        # # Compute target Q-values using the target model and next actions from the online model
        # with torch.no_grad():
        #     target_q_values_next = self.target_model(next_states).gather(1, next_actions)
        #     target_q_values = rewards.unsqueeze(1) + (1.0 - dones.float().unsqueeze(1)) * self.gamma * target_q_values_next
