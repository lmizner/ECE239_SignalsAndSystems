...
class VAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mu = self.mu_layer(encode_output)
        
        logvar = self.logvar_layer(encode_output)
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        encode_in = torch.cat((x_flatten, c), 1)

        encode_out = self.encoder(encode_in)

        mu = self.mu_layer(encode_out)

        logvar = self.logvar_layer(encode_out)

        z = reparametrize(mu, logvar)

        decode_in = torch.cat((z, c), 1)

        x_hat = self.decoder(decode_in)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    r_loss = torch.nn.functional.binary_cross_entropy(x_hat, x, reduction = "sum")

    KL_divergence = -0.5 * torch.sum(1 + logvar - mu**2 - torch.exp(logvar))

    loss = (r_loss + KL_divergence) / N
